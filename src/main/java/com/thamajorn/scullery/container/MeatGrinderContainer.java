...
public class MeatGrinderContainer extends Container {
    ...
    private MeatGrinderTileEntity tileEntity;
    private IWorldPosCallable canInteractWithCallable;
    public FunctionalIntReferenceHolder currentSmeltTime;

    public MeatGrinderContainer(final int windowID, final PlayerInventory playerInv, final MeatGrinderTileEntity tile) {
        ...
        this.addSlot(new SlotItemHandler(tile.getInventory(), 1, 114, 20));
        this.addSlot(new SlotItemHandler(tile.getInventory(), 2, 114, 60));
    ...
    public MeatGrinderContainer(final int windowID, final PlayerInventory playerInv, final PacketBuffer data) {
        this(windowID, playerInv, getTileEntity(playerInv, data));
    }

    private static MeatGrinderTileEntity getTileEntity(final PlayerInventory playerInv, final PacketBuffer data) {
        Objects.requireNonNull(playerInv, "playerInv cannot be null");
        Objects.requireNonNull(data, "data cannot be null");
        final TileEntity tileAtPos = playerInv.player.world.getTileEntity(data.readBlockPos());
        if (tileAtPos instanceof CuttingBoardTileEntity) {
            return (MeatGrinderTileEntity) tileAtPos;
        }
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(canInteractWithCallable, playerIn, registryHandler.GRINDER.get());
    }
